        ...
        if not phase1:
            ...
            for i in range(train_interval):
                ...
                with open(auc_file, "a") as f_phase2:
                    f_phase2.write("{}\n".format(auc))
                print(
                    "{}th evaluation using new data ap: {:.4f} auc: {:.4f}".format(i, ap, auc))
...
if args.local_rank < args.num_gpus:
    ...
    curr_build_graph_time, eval_time = main(phase1_percent, curr_start, phase1=True,
                                            model=model, optimizer=optimizer, mailbox=mailbox, train_interval=train_interval)
    ...
    for i in range(retrain_num):
        ...
        print("current total build graph time: {}".format(total_build_graph_time))
